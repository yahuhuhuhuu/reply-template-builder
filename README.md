...
素朴に「毎回変わる部分を変数にして」と指示すると、モデルは本題の段落をまるごと `{{main_response}}` のような 1 変数にしてしまいます。文法的には正しいテンプレですが、埋めるには結局全文を書くことになり、実用価値がありません。

そのため `EXTRACT_SYSTEM` には粒度の制約を明示しています。
...
- 共通の文言が見つからない箇所を変数の穴で逃げない
- 出力前に「変数を除いた本文が日本語として読めるか」を自己チェックさせる

さらにコード側（`src/claude.js` の `reconcileTemplate`）で、変数表とテンプレ本文のずれを機械的に検出しています。スキーマは各フィールドの型しか保証しないため、「変数表にはあるが本文に出てこない」といった不整合はそのまま通ってしまうためです。検出したら黙って直さず、必ず「抽出時のメモ」に記録して画面に出します。
