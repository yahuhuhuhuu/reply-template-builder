...
3. **分類**: 対応付けたブロックを次の3種に判定する。
   ...
   - variable: 役割も文型も共通で、**差し替わるのが語句だけ**の部分。{{key}} のプレースホルダにする。
   - conditional: 一部のサンプルにしか存在しない、または場合によって文言そのものが入れ替わるブロック。どんな場合に入れるかを condition に書く。
...
# 変数の粒度（最重要）
テンプレの価値は「そのまま送っても8割方成立する文章が残っていること」で決まる。
文をまるごと {{...}} にすると、埋める人が結局全文を書くことになり、テンプレの意味が失われる。

- variable にしてよいのは **語句レベル**（固有名詞・日付・数量・金額・成果物名・状態名など）。目安は1つあたり20文字以内。
- 「〜いたします」「〜でございます」などの述部や文末表現を変数に含めない。述部は必ず地の文（fixed）側に残す。
- **文や段落まるごとを1つの変数にしてはならない。** 本題の回答文であっても、共通の文型を fixed として書き出し、その中の語句だけを変数にする。
  - 悪い例: \`{{main_answer}}\` / \`{{action_taken}}\` / \`{{supplementary_note}}\`
  - 良い例: \`{{deliverable}}を作成のうえ、{{lead_time}}にメールにてお送りいたします。\`
- サンプル間で**文言そのものが入れ替わる二択・多択**（お礼 ↔ お詫び など）は、1つの変数にまとめず、**それぞれ独立した conditional ブロック**にして具体的な文言を残す。
  - 悪い例: \`{{gratitude_or_apology}}\`
  - 良い例: conditional「お礼」= 『このたびは{{inquiry_topic}}についてご連絡いただき、誠にありがとうございます。』 / conditional「お詫び」= 『このたびはご不便をおかけしており、誠に申し訳ございません。』
- **共通の文言が見つからない箇所を、変数で穴として残してはならない。** その場合は逃げずに、サンプルに実際に書かれていた文をそのまま conditional ブロックとして併記し、適用条件を condition に書く。使える文が本文に残ることがテンプレの価値である。
- **全サンプルで値が同一のものは変数にしない**（自社名・部署名・署名など）。実際の文字列をそのまま fixed ブロックに書く。
- 変数の総数は **8個以内** を目安とする。超えそうなときは、粒度が細かすぎないか・文を変数にしていないかを見直す。

# 出力前の自己チェック（必ず実施する）
次の4点を確認し、1つでも当てはまれば作り直してから出力すること。
1. 文や段落まるごとを1つの変数にしていないか（key に answer / response / note / detail / offer / acknowledgement のような「中身が文である」ことを示す語が入っていたら赤信号）。
2. variables に挙げた key が、すべて templateText 中に \`{{key}}\` として出現しているか。出現しない変数を挙げてはならない。
3. templateText 中の \`{{key}}\` が、すべて variables に定義されているか。
4. 変数を除いた templateText が、日本語のメールとして意味の通る文章になっているか。穴だらけで文意が追えないなら、変数を減らして地の文を復元する。
